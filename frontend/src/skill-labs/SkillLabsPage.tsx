      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {paginated.map((lab) => (
            <div
              key={lab.id}
              className="group p-5 rounded-lg border border-border bg-card shadow-sm flex flex-col justify-between hover:border-primary/30 hover:shadow-md transition-all"
            >
              <div>
                {/* Title row */}
              ...
              <button
                className="mt-4 px-4 py-2 rounded-md bg-primary text-primary-foreground font-medium text-sm inline-flex items-center gap-2 hover:bg-primary/90 transition w-full justify-center"
                onClick={() => setRoute(`skill-lab:${lab.id}:${timed ? 'timed' : 'casual'}` as any)}
              >
                <Play className="w-4 h-4" />
                {timed ? 'Start Timed Lab' : 'Start Casual Lab'}
              </button>
            </div>
          ))}
        </div>
