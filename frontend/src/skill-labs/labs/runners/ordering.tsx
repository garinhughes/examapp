...
export function OrderingRunner({ lab, timed = true }: Props) {
  ...
  const doSubmit = useCallback(async () => {
    ...
    await session.finalize(correct, steps.map((s) => s.id).join(','))
  }, [steps, session.finalize])
            ...
            <SortableContext items={steps.map((s) => s.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-2">
                {steps.map((step, idx) => {
                  ...
                  return (
                    <SortableOrderItem key={step.id} id={step.id} disabled={session.submitted} className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${borderCls} bg-card transition`}>
                      ...
                      {session.submitted && step.correctPosition !== idx + 1 && (
