import { Save, X, Play, Pause, Info, BarChart3, BookOpen, Terminal, Minimize2, Maximize2, Flag, Star, Tag } from 'lucide-react'
...
import { clearLabProgress } from '../skill-labs/labs/shared'
...
import { ExamAttemptReview } from './ExamAttemptReview'
...
function ExamAppInner() {
  ...
  const {
    ...
    serverInProgress, setServerInProgress, setSavedExamVersion,
    setSelectedAnswers, setMultiSelectPending, setFlaggedQuestions, setTimeLeft,
    setShowSubmitConfirm, setShowCompleteEarlyConfirm,
  } = useExam()
  ...
  // Immediate cancel from the top-of-page exam banner (no modal, no reason chips).
  async function handleBannerCancelExam() {
    const bannerCode = anySavedExam?.code ?? selected
    const fallbackIds = serverInProgress
      .filter(s => !bannerCode || s.examCode === bannerCode)
      .map(s => s.attemptId)
    const ids = Array.from(new Set([attemptId, ...fallbackIds].filter(Boolean) as string[]))
    for (const id of ids) {
      try { await authFetch(`/attempts/${id}/abandon`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }) } catch {}
    }
    if (bannerCode) {
      try { localStorage.removeItem(`attempt:${bannerCode}`) } catch {}
      try { localStorage.removeItem(`examProgress:${bannerCode}`) } catch {}
    }
    setServerInProgress(prev => prev.filter(s => !ids.includes(s.attemptId) && s.examCode !== bannerCode))
    setSavedExamVersion(v => v + 1)
    setAttemptId(null); setAttemptData(null); setExamStarted(false)
    setSelectedAnswers({}); setMultiSelectPending({}); setFlaggedQuestions(new Set())
    setCurrentQuestionIndex(0); setTimeLeft(null); setPaused(false)
    setShowSubmitConfirm(false); setShowCompleteEarlyConfirm(false); setShowCancelConfirm(false)
  }

  // Skill-lab in-progress state — sourced from SkillLabContext (server-authoritative).
  const { inProgressLab, cancelActive } = useSkillLab()
  const inProgressLabId = inProgressLab?.labId ?? null
  ...
  // When Start exam is clicked from the landing page, examStarted flips true while
  // route is still 'exam-landing'. Transition to 'home' so the runner renders and
  // useRouteSync navigates the URL to /exams/:code/attempt.
  useEffect(() => {
    if (examStarted && route === 'exam-landing') setRoute('home')
  }, [examStarted, route])
  ...
  return (
    ...
    <div className="flex flex-col h-screen w-full overflow-hidden bg-background text-foreground">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* ── Exam in-progress banner — any /exams* route ── */}
        {anySavedExam && !examStarted && !focusMode && (
          route === 'practice' || route === 'exam-landing' || (route === 'home' && !!selected) ||
          route === 'exam-history' || route === 'exam-attempt-review'
        ) && (
          <div className="flex justify-center pl-4 pr-14 md:px-4 py-2.5 border-b border-border bg-card/80 backdrop-blur-sm shrink-0 z-10">
            <div className="flex flex-col items-center gap-1.5">
              <div className="flex items-center gap-2">
                <BookOpen className="w-3.5 h-3.5 text-primary shrink-0" aria-hidden />
                ...
                <span className="hidden sm:inline text-xs text-muted-foreground truncate">{anySavedExam.title} · {anySavedExam.answeredCount}/{anySavedExam.total}</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                <button
                  ...
                  onClick={() => void handleBannerCancelExam()}
                >
        ...
        {/* ── Skill-Lab in-progress banner — list & detail only (NOT runner) ── */}
        {inProgressLabId && !focusMode && (route === 'skill-labs' || route.startsWith('skill-lab-detail')) && (
          <div className="flex justify-center pl-4 pr-14 md:px-4 py-2.5 border-b border-border bg-card/80 backdrop-blur-sm shrink-0 z-10">
            <div className="flex flex-col items-center gap-1.5">
              <div className="flex items-center gap-2">
                <Terminal className="w-3.5 h-3.5 text-primary shrink-0" aria-hidden />
                <span className="font-semibold text-sm text-foreground whitespace-nowrap">Skill Lab in progress</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  className="px-3 py-1 rounded-md bg-primary text-white text-xs inline-flex items-center gap-1.5 shadow-sm hover:bg-primary/90 transition whitespace-nowrap"
                  onClick={() => setRoute(`skill-lab:${inProgressLabId}:${inProgressLab?.timed ? 'timed' : 'casual'}` as any)}
                >
                  <Play className="w-3 h-3" aria-hidden />
                  Resume
                </button>
                <button
                  className="px-3 py-1 rounded-md bg-muted text-muted-foreground border border-border text-xs inline-flex items-center gap-1.5 hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30 transition whitespace-nowrap"
                  onClick={() => {
                    clearLabProgress(inProgressLabId)
                    void cancelActive()
                  }}
                >
                  <X className="w-3 h-3" aria-hidden />
        ...
        {/* ── Discount banner — Overview, Pricing, Basket for non-subscribers ── */}
        {discountActive && !isPaidTier(entitlementTier) && !focusMode && (
          (route === 'home' && !selected) || route === 'pricing' || route === 'basket'
        ) && (
          <div className="flex justify-center pl-4 pr-14 md:px-4 py-2.5 border-b border-border bg-card/80 backdrop-blur-sm shrink-0 z-10">
            <div className="flex items-center gap-3">
              <Tag className="w-3.5 h-3.5 text-primary shrink-0" aria-hidden />
              <span className="text-sm font-semibold text-foreground whitespace-nowrap">Limited-time offer</span>
              <span className="text-xs text-muted-foreground">Save £2/month<span className="hidden sm:inline"> on Pro &amp; Pro Plus</span></span>
              {route !== 'pricing' && (
                <button
        ...
        <div id="exam-scroll-container" className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="container mx-auto max-w-6xl space-y-8">
            {/* Header */}
            <header className={`flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6${focusMode ? ' hidden' : ''}`}>
              <div className="flex flex-col">
                {['practice', 'analytics', 'exam-history', 'exam-landing', 'exam-attempt-review', 'account', 'admin', 'metrics', 'diagrams', 'skill-labs', 'basket', 'feedback'].includes(route) && (
                  <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
                    <span className="text-primary font-extrabold">//</span>
                    {route === 'practice' && 'Practice Exams'}
                    {route === 'analytics' && 'Analytics'}
                    {route === 'exam-history' && 'Exam Score History'}
                    {route === 'exam-landing' && (selectedMeta?.title ?? selected ?? 'Exam')}
                    {route === 'exam-attempt-review' && `${selectedMeta?.title ?? selected ?? 'Exam'} — Attempt Review`}
            ...
            {/* Exam landing page — /exams/:code (dev-guide §16 / 15.2) */}
            {route === 'exam-landing' && <ExamLanding />}

            {/* Per-attempt review — /exams/:code/attempt/:attemptId (dev-guide §16 / 15.4) */}
            {route === 'exam-attempt-review' && <ExamAttemptReview />}

            {/* Practice Exams page */}
            {route === 'practice' && <PracticeExams />}

            {/* Analytics page (legacy /analytics → redirected) and per-exam history page */}
            {(route === 'analytics' || route === 'exam-history') && <AnalyticsView />}
            ...
            {/* Results — Udemy-style summary card. The celebration / rating popup lives in ExamCompleteModal. */}
            {attemptData && typeof attemptData.score === 'number' && (route === 'home' || route === 'exam-attempt-review') && !focusMode && (
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
                    ...
                    return (
                      <div className="relative shrink-0" style={{ width: SIZE, height: SIZE }}>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          ...
                          <span className={`text-[10px] font-semibold uppercase tracking-wide ${passed ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
                  ...
                  <div className="flex-1 min-w-0 w-full">
                    <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-sm">
                      <span><span className="font-semibold tabular-nums">{attemptData.correctCount ?? 0}</span> <span className="text-muted-foreground">/ {attemptData.total ?? 0} correct</span></span>
                      {attemptData.earlyComplete && (
                        <span className="text-xs text-primary">Completed early ({attemptData.answeredCount}/{attemptData.totalQuestions})</span>
                      )}
                      <span className="text-xs text-muted-foreground sm:ml-auto">{attemptData.finishedAt ? new Date(attemptData.finishedAt).toLocaleString() : ''}</span>
                    </div>

                    {/* Domain bars — bar colour does the work, no Strong/Weak labels. */}
                          ...
                          return (
                            <div key={domain}>
                              <div className="flex items-center justify-between text-xs mb-1">
                                ...
                                <span className="text-muted-foreground tabular-nums shrink-0">{vscore}% <span className="opacity-60">({correct}/{total})</span></span>
                              </div>
                              <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                                <div className={`h-full transition-all ${barClass}`} style={{ width: `${vscore}%` }} />
                              </div>
                            </div>
                          )
                        })
    ...
    <TourBubble />
    <PaymentResultModal />
    {examRatingOpen && selected && (
      <RatingModal contentType="exam" contentId={selected} onClose={() => setExamRatingOpen(false)} />
    )}
    {/* Auto-open celebration modal once on finish (dev-guide §16 / 15.11) */}
    <ExamCompleteModal />
    </FeedbackProvider>
  )
}
