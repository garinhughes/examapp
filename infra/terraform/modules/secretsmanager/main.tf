...
# Cognito app client ID (value managed out-of-band)
resource "aws_secretsmanager_secret" "cognito_app_client_id" {
  name        = "${var.project}-cognito-app-client-id"
  description = "Cognito app client ID for ${var.project} (certshack account)"
  tags        = { Project = var.project }
...
# Allow the ECS task execution role to read the client ID secret
resource "aws_secretsmanager_secret_policy" "cognito_app_client_id_policy" {
  secret_arn = aws_secretsmanager_secret.cognito_app_client_id.arn
  policy     = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
        Sid    = "AllowEcsTaskGetClientId",
        Effect = "Allow",
        Principal = { AWS = var.ecs_task_execution_role_arn },
        Action = ["secretsmanager:GetSecretValue", "secretsmanager:DescribeSecret"],
        Resource = aws_secretsmanager_secret.cognito_app_client_id.arn
      }
    ]
  })
}
